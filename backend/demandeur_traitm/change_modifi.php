...
return null;
    ...
    public function updateDemande($id_dm, $type_besoin, $description, $urgence, $date_limite, $filename = null) {
        if ($filename) {
    $stmt = $this->pdo->prepare("UPDATE demande SET typedebesoin = ?, description_dm = ?, urgence_dm = ?, date_limite_dm = ?, piece_jointe_dm = ? WHERE id_dm = ?");
    return $stmt->execute([$type_besoin, $description, $urgence, $date_limite, $filename, $id_dm]);
} else {
    $stmt = $this->pdo->prepare("UPDATE demande SET typedebesoin = ?, description_dm = ?, urgence_dm = ?, date_limite_dm = ? WHERE id_dm = ?");
