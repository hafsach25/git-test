...
<div class="taux-container">
    ...
    <button type="submit" class="action-btn btn-accept" id="<?= htmlspecialchars($demande['id_dm']) ?>">Valider</button>


    <button type="submit" class="action-btn btn-reject" id="<?= htmlspecialchars($demande['id_dm']) ?>">Rejeter</button>
...
<?php endif; ?>
                        <button class="action-btn btn-detail" id="<?= htmlspecialchars($demande['id_dm']) ?>">Détails</button>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
...
new Chart(ctx, { type: 'line', data: evolutionData, options: { responsive: true, plugins: { legend: { position: 'top' } } } });
